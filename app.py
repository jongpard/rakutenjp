# -*- coding: utf-8 -*-
"""
Rakuten Japan Beauty Daily 1~160
- GitHub Actions에서 403/빈DOM 나와서 디버그용 HTML/PNG를 항상 남기는 버전
- 1페이지: https://ranking.rakuten.co.jp/daily/100939/
- 2페이지: https://ranking.rakuten.co.jp/daily/100939/p=2/
"""
...
def normalize_folder_id(raw: str) -> str:
    if not raw:
        return ""
    s = raw.strip()
    m = re.search(r"/folders/([a-zA-Z0-9_-]{10,})", s) or re.search(
...
def translate_ja_to_ko_batch(lines: List[str]) -> List[str]:
    flag = os.getenv("SLACK_TRANSLATE_JA2KO", "0").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    def _try_t(srcs: List[str]) -> List[str]:
        try:
            ...
            print("[translate] googletrans fail:", e)
        try:
            ...
            print("[translate] deep-translator fail:", e)
        return ["" for _ in srcs]

    ja_tr = _try_t(ja_only)
    it = iter(ja_tr)
    out = []
    for t in texts:
...
def fetch_by_playwright_rakuten() -> List[Product]:
    ...
    with sync_playwright() as p:
        ...
        for idx, url in enumerate(RAKUTEN_URLS, start=1):
            ...
            # 💾 디버그: HTML + 스크린샷 저장
            html_path = f"data/debug/rakuten_p{idx}.html"
            png_path = f"data/debug/rakuten_p{idx}.png"
            try:
            ...
            # 우선 순위 1: li.rnkRanking_item
            elements = page.query_selector_all("li.rnkRanking_item")
            if not elements:
                # 플랜 B: ul.rnkRanking_list > li
                elements = page.query_selector_all("ul.rnkRanking_list > li")
            ...
            for el in elements:
                try:
                    ...
                    if not name_a:
                        # 혹시 그냥 a 하나만 있는 경우
                        name_a = el.query_selector("a[href*='item.rakuten.co.jp']")
                    ...
                    # 가격
                    price_span = el.query_selector(
                        ".rnkRanking_itemPrice span.important"
                    )
                    if not price_span:
                        price_span = el.query_selector(".rnkRanking_itemPrice")
...
def build_sections(
    df_today: pd.DataFrame, df_prev: Optional[pd.DataFrame]
) -> Dict[str, List[str]]:
    ...
    for _, r in top10.iterrows():
        ...
        if prev_index is not None:
            ...
            if key in prev_index.index and pd.notnull(prev_index.loc[key, "rank"]):
                ...
                marker = "(New) "
        price_txt = (
            fmt_currency_jpy(r.get("price")) if pd.notnull(r.get("price")) else "￥0"
        )
    ...
    for k in common:
        ...
        if drop > 0:
            row = tN.loc[k]
            movers.append(
                (drop, cr, pr, f"- {_link(row)} {pr}위 → {cr}위 (↓{drop})", _plain_name(row))
            )
...
def main():
    ...
    if folder:
        try:
            ...
            print("[INFO] 드라이브 업로드 완료:", file_today)
            df_prev = drive_download_csv(svc, folder, build_filename(y_yes))
            if df_prev is None:
                print("[INFO] 전일 파일 없음:", build_filename(y_yes))
            else:
                print("[INFO] 전일 파일 로드 성공")
        except Exception as e:
            print("[Drive 오류]", e)
            traceback.print_exc()
    else:
        print("[INFO] GDRIVE_FOLDER_ID 없음 → 업로드 생략")

    # 0개라도 디버그는 남겨야 하니까 일단 슬랙은 보냄
    if len(products) < 20:
        slack_post(
            "*라쿠텐 뷰티 랭킹 수집 경고*\n0개 혹은 너무 적게 수집되었습니다.\nGitHub artifact의 data/debug/ 아래 html/png 확인하세요."
        )
        # 그래도 실패로 끝내서 눈에 띄게
        raise RuntimeError(
            "라쿠텐 제품 카드가 너무 적게 수집되었습니다. data/debug/ 아래 HTML/Screenshot 확인"
        )
