# -*- coding: utf-8 -*-
"""
Rakuten JP Beauty(100939) Daily Rank Top N
- ScraperAPI(JP) + render=False → 0개면 True 재시도(페이지별)로 크레딧 절약
- TOP3/그 이후 DOM 차이를 흡수: itemName a 기준으로 근접 랭크/가격/샵 탐색
- 페이지네이션으로 충분히 수집 (1페이지=1~80, 2페이지=81~160)
- CSV 저장 + (옵션) Google Drive 업로드
- Slack 리포트: TOP10 일본어 + 한국어 번역(배치), 급하락/인&아웃, 변동0은 "(-)"
- 전일 CSV가 name/product_name 어느 형식이든 호환
"""
...
# 대상 페이지들
BASE_PATH = "https://ranking.rakuten.co.jp/daily/100939/"
RANK_URLS = [BASE_PATH, BASE_PATH + "p=2/"]  # 1~80, 81~160
...
def nearest_container_with_rank(el) -> Optional[BeautifulSoup]:
    cur = el
    for _ in range(8):  # 위로 8단계까지 탐색
        if not cur: break
        if cur.select_one(".rnkRanking_dispRank"): return cur
        cur = cur.parent
    return None
...
def parse_rank_page(html: str, add_offset: int) -> List[Dict]:
    ...
    for a in soup.select("div.rnkRanking_itemName a"):
        cont = nearest_container_with_rank(a)
        if not cont: continue

        # rank
        r_el = cont.select_one(".rnkRanking_dispRank")
        rk_txt = clean(r_el.get_text()) if r_el else ""
        m_r = re.search(r"\d+", rk_txt)
        if not m_r: continue
        rank = int(m_r.group(0))
        ...
        # price
        pr_el = cont.select_one(".rnkRanking_price")
        pr_txt = clean(pr_el.get_text()) if pr_el else ""
        m = YEN_RE.search(pr_txt or "")
        price = int(m.group(1).replace(",", "")) if m else None

        # shop → brand
        sh_a = cont.select_one(".rnkRanking_shop a")
        shop = clean(sh_a.get_text()) if sh_a else ""
        brand = brand_from_shop(shop)

        rows.append({
            "rank": rank + (0 if add_offset == 0 else 0),  # add_offset은 현재 0/80 구분 불필요
            "product_name": name, "price": price, "url": href, "shop": shop, "brand": brand
        })
...
def fetch_all() -> List[Dict]:
    ...
    for url in RANK_URLS:
        ...
        if SAVE_DEBUG:
            open(f"{DBG_DIR}/rakuten_{'p2' if add else 'p1'}_r{'1' if render_first else '0'}.html","w",encoding="utf-8").write(html)
        rows = parse_rank_page(html, add)

        if len(rows) < 50 and not render_first:
            # 페이지만 렌더 ON 재시도
            html = scraperapi_get(url, render=True)
            if SAVE_DEBUG:
                open(f"{DBG_DIR}/rakuten_{'p2' if add else 'p1'}_r1_retry.html","w",encoding="utf-8").write(html)
            rows = parse_rank_page(html, add)

        allrows.extend(rows)
        time.sleep(0.6)

    allrows = [r for r in allrows if 1 <= r["rank"] <= 10000]
    allrows = sorted(allrows, key=lambda r: r["rank"])[:MAX_RANK]
    return allrows

# ================= 번역(배치) =================
def translate_ja2ko_batch(texts: List[str]) -> List[str]:
    if not DO_TRANSLATE or not texts: return texts
    try:
        # deep-translator (pip 설치되어 있음)
        from deep_translator import GoogleTranslator
        tr = GoogleTranslator(source="ja", target="ko")
        # batch translate는 길면 분할
        out = []
        chunk, size = [], 40
        for t in texts:
            chunk.append(t)
            if len(chunk) >= size:
                out.extend(tr.translate_batch(chunk)); chunk = []
        if chunk: out.extend(tr.translate_batch(chunk))
        return out
    except Exception as e:
        print("[번역 경고]", e);  return texts  # 실패 시 원문 리턴
...
def build_sections(df_today: pd.DataFrame, df_prev: Optional[pd.DataFrame]) -> Dict[str, list]:
    ...
    ja_names = top10[name_col_today].astype(str).tolist()
    ko_names = translate_ja2ko_batch(ja_names)
    lines = []
    prev_idx = None
    if name_col_prev:
    ...
    for i, (_, r) in enumerate(top10.iterrows()):
        ...
        if prev_idx is not None:
            ...
            if k in prev_idx.index and pd.notnull(prev_idx.loc[k, "rank"]):
                pr = int(prev_idx.loc[k, "rank"]); cr = int(r["rank"]); diff = pr - cr
                mark = f"(↑{diff}) " if diff>0 else (f"(↓{abs(diff)}) " if diff<0 else "(-) ")
            else:
                mark = "(New) "
        price_txt = f"￥{int(r['price']):,}" if pd.notnull(r.get("price")) else "￥0"
        ja = ja_names[i]; ko = ko_names[i] if i < len(ko_names) else ja
        link = f"<{r['url']}|{slack_escape(ja)}>"
